...
- **The container images shipped without build provenance, so there was no way to establish how a pulled image was built.** The release attested the downloadable archives but not the images, because `actions/attest-build-provenance` identifies an image by digest rather than by path and the digest was not being read out of goreleaser's artefact manifest. Both images are now attested and the attestation is pushed to the registry alongside the image, so `gh attestation verify oci://ghcr.io/hexamatic/pihole-mcp:X.Y.Z --repo hexamatic/pihole-mcp` resolves it with no download. This is a different guarantee from the cosign signature the images already carried, and worth running as well as rather than instead of it: the signature proves who signed the image, the attestation is a signed statement of how it was built. Absent since images were first published.
...
- **The admin password's hash has always reached the MCP client through `pihole_config_get`, and nothing said so.** Pi-hole's configuration API masks the password and the TOTP secret but returns `webserver.api.pwhash` in full, and neither `pihole_config_get` nor `pihole_config_get_value` has ever filtered it, so any request that touched the `webserver` section handed the hash to the model on the other side of the client. It is a memory-hard BALLOON-SHA256 hash, not the password, but a weak password can be guessed against it offline. SECURITY.md now has a Data Handling section that states this alongside everything else the server returns about the people and devices on a network (query history, the device inventory, other users' Pi-hole sessions and backup archives) and shows how to withhold each category with `PIHOLE_TOOLSETS`; leaving out the `config` toolset withholds the hash. Present since v0.1.0.
